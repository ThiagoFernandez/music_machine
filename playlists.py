...
def pick_playlist():
    ...
    auxiliar.show_options(all_playlist)
    playlist = auxiliar.validate_number(all_playlist)
    if playlist == -1:
        return -1
    else:
...
def rename_playlist():
    ...
    if playlist == -1:
        ...
        new_name = auxiliar.validate_string([playlist], "Write the new name of the playlist", 1)
        if new_name == -1:
            return -1
        else:
